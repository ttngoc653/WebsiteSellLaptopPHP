...
<div>
	<div class="panel panel-default">
  		...
  		<div class="panel-body">
    		<?php $rs=ConnectQuery("select * from san_pham order by masp desc limit 5");
    		while ($row=$rs->fetch_assoc()) {
    				?><div style="margin: 5px;float: left;width: 150px;text-align: center;">
				<a href="index.php?act=chitiet&masp=<?php echo $row['masp']; ?>">
					<img width="150px" height="150px" src="./image/<?php echo $row['masp']."/".$row['icon']; ?>"/>
				...
				<div style="font:italic bold 12px arial;margin-right: 30px;	margin-bottom: 10px;color:blue;	text-align: center;">
					<input style="border-width: 0px;" type="submit" name="them" value="Thêm vào giỏ">
				</div>
						</div>
			<?php 
				}
	...
	<div class="panel panel-default">
  		...
  		<div class="panel-body">
    		<?php $rs=ConnectQuery("select san_pham.masp,san_pham.tensp,san_pham.hangsx,san_pham.gia,san_pham.icon,sum(chi_tiet_don_hang.soluongsp) FROM san_pham,chi_tiet_don_hang WHERE san_pham.masp=chi_tiet_don_hang.masp GROUP BY san_pham.masp order by sum(soluongsp) desc LIMIT 5");
    		while ($row=$rs->fetch_assoc()) {
    				?><div style="margin: 5px;float: left;width: 150px;text-align: center;">
				<a href="index.php?act=chitiet&masp=<?php echo $row['masp']; ?>">
					<img width="150px" height="150px" src="./image/<?php echo $row['masp']."/".$row['icon']; ?>"/>
	...
	<div class="panel panel-default">
  		...
  		<div class="panel-body">
    		<?php $rs=ConnectQuery("select * from san_pham order by luotview desc limit 5");
    			while ($row=$rs->fetch_assoc()) {
    				?><div style="margin: 5px;float: left;width: 150px;text-align: center;">
				<a href="index.php?act=chitiet&masp=<?php echo $row['masp']; ?>">
					<img width="150px" height="150px" src="./image/<?php echo $row['masp']."/".$row['icon']; ?>"/>
